...
# use the latest params from optimizer (safer)
params = get_params(opt_state)

# build a dict of numpy arrays to save
save_dict = {}
# params[0] is list of weight matrices for each layer
for i, arr in enumerate(params[0]):
    save_dict[f"A_{i}"] = onp.array(arr)   # convert DeviceArray -> numpy

# params[1] is list of bias vectors for each layer
for i, arr in enumerate(params[1]):
    save_dict[f"B_{i}"] = onp.array(arr)

model_path = f"models/{seed}_{'-'.join(map(str, sizes))}.npz"
onp.savez(model_path, **save_dict)
print("Saved model to", model_path)

# model_path = "models/" + str(seed) + "_" + "-".join(map(str,sizes))
# onp.savez(model_path, A=params[0], B=params[1])

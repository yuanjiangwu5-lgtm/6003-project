...
def graph_solve(all_ratios, all_criticals, expected_neurons, LAYER, debug=False):
    ...
    while previous_num_components > len(sorted_components):
        ...
        try:
            if pool:
                ...
                results = [ratio_normalize(d) for d in datas]
        except Exception:
            results = [ratio_normalize(d) for d in datas]
    ...
    if len(resulting_rows)+skips_because_of_nan < expected_neurons and len(all_ratios) < DEAD_NEURON_THRESHOLD:
        ...
        # ---- PATCH START ----
        try:
            part_rows = np.array(resulting_rows)
            part_examples = np.array(resulting_examples)
        except ValueError:
            # ragged list fallback
            part_rows = np.array(resulting_rows, dtype=object)
            part_examples = np.array(resulting_examples, dtype=object)

        raise AcceptableFailure(partial_solution=(part_rows, part_examples))
        # ---- PATCH END ----
